...
namespace Romanesco.Test.EditorComponents
{
	public class EditorTest
	{
		...
		public void 与えたIProjectSaveServiceでプロジェクトを保存できる()
		{
			...
			editor.SaveAsync().Wait();

			saveService.Verify(x => x.SaveAsync(), Times.Once);
		}

		[Fact]
		public void 保存時にステートのOnSaveが呼ばれる()
		{
			var saveService = GetMockSaveService();
			var editorState = GetMockEditorStateToSave(saveService);
			var editor = new Editor(neverEditorStateChanger, editorState.Object, new CommandAvailability());
			...
			editorState.Verify(x => x.OnSave(), Times.Once);
		}
		...
		[Fact]
		public void 上書き保存時にステートのOnSaveAsが呼ばれる()
		{
			var saveService = GetMockSaveService();
			var editorState = GetMockEditorStateToSave(saveService);
			var editor = new Editor(neverEditorStateChanger, editorState.Object, new CommandAvailability());
			...
			editorState.Verify(x => x.OnSaveAs(), Times.Once);
		}

		private static Mock<IEditorState> GetMockEditorStateToSave(Mock<IProjectSaveService> saveService)
		{
			var editorState = new Mock<IEditorState>();
			editorState.Setup(x => x.GetSaveService())
				.Returns(saveService.Object);
			editorState.Setup(x => x.OnSave())
				.Callback(() => { });
